...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: `${d.terms.title} — Metis LLC` };
}
    ...
    <PageShell lang={lang} dict={d}>
      <article className="support-page">
        <header className="support-header">
          <h1 className="support-title font-serif">{d.terms.title}</h1>
          <p className="support-lead font-mono">{d.terms.lastUpdated}</p>
        </header>
        {d.terms.sections.map((section) => (
